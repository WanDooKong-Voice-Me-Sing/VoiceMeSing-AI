...
def click_train(
    ...
    # config생성  모델학습을 위한 파일리스트 준비, 구성파일저장, gpu, 프리트레인 모델 정보 로깅
...
def save_origin_music(user_id, origin_voice, model_name):

    user_path = f"source/{user_id}"
    if 'app' in now_dir: base_path = os.path.join(now_dir, user_path)
    folders = ['origin', 'voice', 'inst']
...
def voice_extraction(input, save_vocal, save_ins):
    print("함수 호출 전")
    try:
        vc_output4 = uvr(model_name=uvr5_names[1], inp_root=input, save_root_vocal=save_vocal, paths= "", save_root_ins=save_ins, agg=10, format0="wav")
        # 결과 출력
    ...
    print(f"{uvr5_names}")
    print(f"Selected model: {uvr5_names[1]}")


def mixing(vocal_path, inst_path, output_path):
    if 'app' in now_dir: 
        vocal_path = os.path.join(now_dir, vocal_path)
        inst_path = os.path.join(now_dir, inst_path)
        output_path = os.path.join(now_dir, output_path)
